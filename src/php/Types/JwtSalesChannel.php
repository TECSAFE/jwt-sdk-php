...
class JwtSalesChannel implements JsonSerializable
{
    ...
    public static function fromJson(string|array $data): JwtSalesChannel
    {
        if (!is_array($data)) {
            $data = json_decode($data, true);
        }

        return new JwtSalesChannel($data['exp'], $data['iat'], $data['iss'], $data['jti'], JwtSalesChannelMeta::fromJson($data['meta']), $data['nbf'], $data['sub'], JwtType::from($data['type']));
    }
