...
class JwtInternal implements JsonSerializable
{
    ...
    public static function fromJson(string|array $data): JwtInternal
    {
        if (!is_array($data)) {
            $data = json_decode($data, true);
        }

        return new JwtInternal($data['exp'], $data['iat'], $data['iss'], $data['jti'], JwtInternalMeta::fromJson($data['meta']), $data['nbf'], $data['sub'], JwtType::from($data['type']));
    }
